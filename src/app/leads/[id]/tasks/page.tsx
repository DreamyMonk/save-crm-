...
export default function LeadTasksPage() {
  ...
  function addTask(event: FormEvent<HTMLFormElement>) {
    ...
    setState((currentState) => ({
      ...currentState,
      leads: currentState.leads.map((item) => (item.id === lead.id ? { ...item, updatedAt: new Date().toISOString(), tasks: [...item.tasks, task] } : item)),
    }));
    setTaskTitle("");
  }
  ...
  function addNote(event: FormEvent<HTMLFormElement>) {
    ...
    setState((currentState) => ({
      ...currentState,
      leads: currentState.leads.map((item) => (item.id === lead.id ? { ...item, updatedAt: new Date().toISOString(), notes: [...item.notes, note] } : item)),
    }));
    setNoteBody("");
  }
