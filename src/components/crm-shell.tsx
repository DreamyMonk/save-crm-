...
function ShellFrame({ children, email, uid }: { children: React.ReactNode; email: string | null; uid: string }) {
  ...
  return (
        ...
        {hasAccess ? (
          <nav className="fixed inset-x-0 bottom-0 z-40 flex gap-1 overflow-x-auto border-t border-[#dfe7d7] bg-white/96 px-2 py-2 shadow-[0_-12px_24px_rgba(15,23,42,0.08)] backdrop-blur lg:hidden">
            {visibleNav.map((item) => {
              const Icon = item.icon;
              const matchHref = item.matchHref ?? item.href;
              const active = item.href.includes("#") ? false : matchHref === "/" ? pathname === "/" : pathname.startsWith(matchHref);
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`flex min-w-[74px] flex-col items-center justify-center gap-1 rounded-lg px-2 py-2 text-[11px] font-semibold ${
                    active ? "bg-[#003CBB] text-white" : "text-[#526258] hover:bg-[#eef4ff] hover:text-[#003CBB]"
                  }`}
                >
                  <Icon size={17} />
...
export function PageHeader({
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-[#dfe7d7] bg-[#ffffff]/92 px-4 py-3 backdrop-blur md:px-8 md:py-4">
